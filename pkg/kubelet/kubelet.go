	...
	if kubeCfg.ContainerRuntime != "rkt" && kubeCfg.EnableCRI {
		...
		case "docker":
			...
			ds, err := dockershim.NewDockerService(klet.dockerClient, kubeCfg.SeccompProfileRoot, kubeCfg.PodInfraContainerImage,
				streamingConfig, &pluginSettings, kubeCfg.RuntimeCgroups, kubeCfg.CgroupDriver, dockerExecHandler, kubeCfg.LoggerType, kubeCfg.LoggerConfig)
			if err != nil {
				return nil, err
			}
		...
		case "docker":
			...
			runtime.AdaptorImageName = kubeCfg.AdaptorImageName
			klet.containerRuntime = runtime
